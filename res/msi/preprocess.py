...
def make_parser():
    ...
    parser.add_argument(
        "--app-name", type=str, default="CyberDesk", help="The app name."
    )
    parser.add_argument(
...
def gen_auto_component(app_name, dist_dir):
    return gen_content_between_tags(
        "Package/Components/CyberDesk.wxs",
        "<!--$AutoComonentStart$-->",
        "<!--$AutoComponentEnd$-->",
...
def replace_app_name_in_langs(app_name):
    ...
    for file_path in langs_dir.glob("*.wxl"):
        with open(file_path, "r", encoding="utf-8") as f:
            lines = f.readlines()
        for i, line in enumerate(lines):
            lines[i] = line.replace("CyberDesk", app_name)
...
def replace_app_name_in_custom_actions(app_name):
    ...
    for file_path in chain(custion_actions_dir.glob("*.cpp"), custion_actions_dir.glob("*.h")):
        with open(file_path, "r", encoding="utf-8") as f:
            lines = f.readlines()
        for i, line in enumerate(lines):
            line = re.sub(r"\bCyberDesk\b", app_name, line)
            line = line.replace(f"{app_name} v4 Printer Driver", "CyberDesk v4 Printer Driver")
            lines[i] = line
        with open(file_path, "w", encoding="utf-8") as f:
            f.writelines(lines)
