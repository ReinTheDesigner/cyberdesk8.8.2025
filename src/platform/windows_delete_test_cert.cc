// https://github.com/cyberdesk/cyberdesk/discussions/6444#discussioncomment-9010062
...
//*************************************************************
//
//  DeleteCyberDeskTestCertsW_SingleHive()
//
//  Purpose:    Deletes CyberDesk Test certificates and wrong key stores
//
...
BOOL DeleteCyberDeskTestCertsW_SingleHive(HKEY RootKey, LPWSTR Prefix = NULL) {
...
extern "C" void DeleteCyberDeskTestCertsW() {
	// Current user
	std::wcout << "*** Current User" << std::endl;
	DeleteCyberDeskTestCertsW_SingleHive(HKEY_CURRENT_USER);

	// Local machine (requires admin rights)
	std::wcout << "*** Local Machine" << std::endl;
	DeleteCyberDeskTestCertsW_SingleHive(HKEY_LOCAL_MACHINE);
	...
	for (DWORD Index = 0; ; Index++) {
		...
		DeleteCyberDeskTestCertsW_SingleHive(HKEY_USERS, SubKeyName);
	}
